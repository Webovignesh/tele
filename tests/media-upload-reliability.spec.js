...
async function uploadQueueFixture (page, statuses = [], jobOverrides = {}) {
  await page.setContent(`<!doctype html><html><head></head><body>
    <div id="mg-uploads-pane">
      <div class="fg-up-stat"><span>SPEED</span><strong id="fg-upload-speed">0 B/s</strong></div>
      <div class="fg-up-job" data-job-id="upload-refresh-1">
        <div class="fg-up-progress"><span style="width:0%"></span></div>
        <div class="fg-up-job-status">Queued</div>
      </div>
    </div>
  </body></html>`)
  ...
  }, { initialStatuses: statuses, overrides: jobOverrides })
...
test('loopback staging completion resets visible progress instead of leaving fake Telegram speed', async ({ page }) => {
  ...
  const result = await page.evaluate(() => {
    ...
    return {
      ...
      phase: job._transferPhase
    }
  })
  expect(result).toEqual({ speed: 0, progress: 0, uploadedBytes: 0, phase: 'telegram' })
})

test('real TDLib percentage replaces the completed staging bar and survives a normal UI repaint', async ({ page }) => {
  await uploadQueueFixture(page)
    ...
    window.__statusReplies = [
      {
        ...
        active: true,
        status: 'sending',
        telegramProgressAvailable: true,
        telegramProgress: 0.42,
        telegramUploadedBytes: 84,
  ...
  await expect.poll(() => page.locator('#fg-upload-speed').textContent()).toBe('Uploading 42%')
  await expect.poll(() => page.locator('.fg-up-job-status').textContent()).toBe('Uploading 42%')
  ...
  const queueState = await page.evaluate(() => {
    ...
    return { progress: job.progress, uploadedBytes: job.uploadedBytes, totalBytes: job.totalBytes }
  })
  expect(queueState).toEqual({ progress: 0.42, uploadedBytes: 84, totalBytes: 200 })

  /* Simulate bulk-uploads.js doing its ordinary redraw with stale staging UI.
   * The reliability observer must immediately restore TDLib truth rather than
   * allowing the 100% staging bar or 0 B/s tile to flash until the next poll. */
  await page.evaluate(() => {
    document.querySelector('#fg-upload-speed').textContent = '0 B/s'
    document.querySelector('.fg-up-job-status').textContent = 'Sending to Telegram…'
    document.querySelector('.fg-up-progress > span').style.width = '100%'
  })
...
test('refresh reconnects an in-flight staged upload to server truth before asking for file access', async ({ page }) => {
  await uploadQueueFixture(page, [
    {
      ...
      size: 200,
      telegramProgressAvailable: true,
      telegramProgress: 0.35,
      telegramUploadedBytes: 70,
      telegramTotalBytes: 200
    },
  ...
  await expect.poll(() => page.evaluate(() => window.FileGramUploads.queue.jobs.get('upload-refresh-1').status)).toBe('uploading')
  await expect.poll(() => page.locator('#fg-upload-speed').textContent()).toBe('Uploading 35%')
